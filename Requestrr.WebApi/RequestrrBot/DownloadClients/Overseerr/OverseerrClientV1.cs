...
{
    public class OverseerrClientV1 : IMovieRequester, IMovieSearcher, ITvShowSearcher, ITvShowRequester
    {
        private IHttpClientFactory _httpClientFactory;
        private readonly ILogger<OverseerrClientV1> _logger;
        private readonly OverseerrDownloadClientSettings _settings;
        ...
        public OverseerrClientV1(IHttpClientFactory httpClientFactory, ILogger<OverseerrClientV1> logger, OverseerrDownloadClientSettings settings, OverseerrTvShowCategory tvShowCategory, OverseerrMovieCategory movieCategory)
        {
            _httpClientFactory = httpClientFactory;
            _logger = logger;
            _settings = settings;
            _tvShowCategory = tvShowCategory;
            _movieCategory = movieCategory;
        }

        public static async Task TestConnectionAsync(HttpClient httpClient, ILogger<OverseerrClientV1> logger, OverseerrTestSettings settings)
        {
        ...
        public static async Task<RadarrServiceSettings> GetRadarrServiceSettingsAsync(HttpClient httpClient, ILogger<OverseerrClientV1> logger, OverseerrTestSettings settings)
        {
        ...
        public static async Task<SonarrServiceSettings> GetSonarrServiceSettingsAsync(HttpClient httpClient, ILogger<OverseerrClientV1> logger, OverseerrTestSettings settings)
        {
        ...
        public async Task<IReadOnlyList<Movie>> SearchMovieAsync(MovieRequest request, string movieName)
        {
            try
            {
                ...
                return movies.Select(x => ConvertMovie(x, _movieCategory.Is4K ? x.MediaInfo?.Status4k : x.MediaInfo?.Status)).ToArray();
            }
            catch (System.Exception ex)
            {
        ...
        public async Task<Movie> SearchMovieAsync(MovieRequest request, int theMovieDbId)
        {
            try
            {
                ...
                return ConvertMovie(movie, _movieCategory.Is4K ? movie.MediaInfo?.Status4k : movie.MediaInfo?.Status);
            }
            catch (System.Exception ex)
            {
        ...
        async Task<MovieRequestResult> IMovieRequester.RequestMovieAsync(MovieRequest request, Movie movie)
        {
            try
            {
                ...
                var overseerrUser = await FindLinkedOverseerUserAsync(request.User.UserId, request.User.Username, _movieCategory.DefaultApiUserId);

                if (!_movieCategory.UseOverseerrSettings)
                {
                    Permission[] permissions = new[] { Permission.AUTO_APPROVE, Permission.AUTO_APPROVE_MOVIE };

                    if (_movieCategory.Is4K)
                    {
                    ...
                    if (overseerrUser != null)
                    {
                        ...
                        if (HasPermission(permissions, userPermissions.Permissions, PermissionCheckOptions.OR))
                        {
                            response = await HttpPostAsync(null, $"{BaseURL}request", JsonConvert.SerializeObject(new
                            {
                                ...
                                mediaType = "movie",
                                is4k = _movieCategory.Is4K,
                                serverId = _movieCategory.ServiceId,
                                profileId = _movieCategory.ProfileId,
                                rootFolder = _movieCategory.RootFolder,
                                tags = JToken.FromObject(_movieCategory.Tags),
        ...
        public async Task<TvShow> GetTvShowDetailsAsync(TvShowRequest request, int theTvDbId)
        {
            try
            {
                ...
                return ConvertTvShow(tvShow, _tvShowCategory.Is4K ? tvShow.MediaInfo?.Status4k : tvShow.MediaInfo?.Status);
            }
            catch (System.Exception ex)
            {
        ...
        async Task<TvShowRequestResult> ITvShowRequester.RequestTvShowAsync(TvShowRequest request, TvShow tvShow, TvSeason season)
        {
            try
            {
                ...
                HttpResponseMessage response = null;
                var overseerrUser = await FindLinkedOverseerUserAsync(request.User.UserId, request.User.Username, _tvShowCategory.DefaultApiUserId);

                if (!_tvShowCategory.UseOverseerrSettings)
                {
                    Permission[] permissions = new[] { Permission.AUTO_APPROVE, Permission.AUTO_APPROVE_TV };

                    if (_tvShowCategory.Is4K)
                    {
                    ...
                    if (overseerrUser != null)
                    {
                        ...
                        if (HasPermission(permissions, userPermissions.Permissions, PermissionCheckOptions.OR))
                        {
                            response = await HttpPostAsync(null, $"{BaseURL}request", JsonConvert.SerializeObject(new
                            {
                                ...
                                is4k = _tvShowCategory.Is4K,
                                serverId = _tvShowCategory.ServiceId,
                                profileId = _tvShowCategory.ProfileId,
                                languageProfileId = _tvShowCategory.LanguageProfileId,
                                rootFolder = _tvShowCategory.RootFolder,
                                tags = JToken.FromObject(_tvShowCategory.Tags),
                                userId = int.Parse(overseerrUser),
                            }));
                        }
                        else
        ...
        private static async Task<HttpResponseMessage> HttpGetAsync(HttpClient client, OverseerrDownloadClientSettings settings, string url)
        {
        ...
        private static string GetBaseURL(OverseerrDownloadClientSettings settings)
        {
            return GetBaseURL(settings.UseSSL, settings.Hostname, settings.Port, settings.Version);
        }

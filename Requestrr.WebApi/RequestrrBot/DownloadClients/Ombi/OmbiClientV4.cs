...
{
    public class OmbiClientV4 : IMovieRequester, IMovieSearcher, ITvShowSearcher, ITvShowRequester
    {
        private IHttpClientFactory _httpClientFactory;
        private readonly ILogger<OmbiClient> _logger;
        private readonly OmbiDownloadClientSettings _settings;
        private readonly OmbiMovieCategory _movieCategory;
        private readonly OmbiTvShowCategory _tvShowCategory;
        private string BaseURL => GetBaseURL(_settings);

        public OmbiClientV4(IHttpClientFactory httpClientFactory, ILogger<OmbiClient> logger, OmbiDownloadClientSettings settings, OmbiTvShowCategory tvShowCategory, OmbiMovieCategory movieCategory)
        {
        ...
        public static async Task TestConnectionAsync(HttpClient httpClient, ILogger<OmbiClient> logger, OmbiDownloadClientSettings settings)
        {
        ...
        public async Task<MovieRequestResult> RequestMovieAsync(MovieRequest request, Movie movie)
        {
            ...
            while (retryCount <= 5)
            {
                try
                {
                    var ombiUser = await FindLinkedOmbiUserAsync(_tvShowCategory.ApiUsername, request.User.UserId, request.User.Username);
        ...
        public async Task<OmbiUser> FindLinkedOmbiUserAsync(string apiUsername, string requesterUniqueId, string requesterUsername)
        {
            ...
            if (ombiUser == null && !string.IsNullOrEmpty(apiUsername))
            {
                ombiUser = FindDefaultApiUserAsync(allOmbiUsers, apiUsername, requesterUsername);
        ...
        private OmbiUser FindDefaultApiUserAsync(JSONOmbiUser[] allOmbiUsers, string apiUsername, string requesterUsername)
        {
            OmbiUser ombiUser = null;

            var defaultApiUser = allOmbiUsers.FirstOrDefault(x => x.userName.Equals(apiUsername, StringComparison.InvariantCultureIgnoreCase));
        ...
        private static string GetBaseURL(OmbiDownloadClientSettings settings)
        {
        ...
        private static async Task<HttpResponseMessage> HttpGetAsync(HttpClient client, OmbiDownloadClientSettings settings, string url)
        {
        ...
        private async Task<HttpResponseMessage> HttpPostAsync(OmbiUser ombiUser, string url, string content)
        {
            ...
            postRequest.Headers.Add("ApiKey", _settings.ApiKey);
